...
# 进一步了解 ATT 网络架构
  ...
  这是历史上第一次去中心化的基础设施具有自我进化的特性，它将能够与拥有领导者和董事会的商业组织运营的专有中心化基础设施竞争。
  `

## 概述

  `NNS 的目的是允许以开放、分散和安全的方式管理 ATT 网络。它可以完全控制网络的各个方面。例如，它可以升级节点机器使用的协议和软件；它可以创建新的元网区块链以增加网络容量；它可以拆分元网进行负载均衡；它可以调配经济参数，控制用户必须为计算能力支付多少费用；并且，在极端情况下，它可以冻结恶意智能合约以保护网络；以及许多其他事情。 NNS 的工作方式是接受提案，并根据网络参与者创建的“神经元”的投票活动决定采用或拒绝它们。
  参与者还使用神经元来提交新提案。提交后，提案要么被采纳，要么被拒绝，提案立即发生，也可以延迟一段时间，具体取决于神经元的整体投票方式。每个提案都是特定“提案类型”的一个实例。对于每种类型的提议都有相应的系统函数，每当采用该类型的提议时，它都会调用该函数。当一个提议被 NNS 采纳时，它通过从提议的内容中提取信息来填充参数来调用相应的系统函数。每种类型的提案都属于一个特定的“提案主题”，例如“#节点管理”或“#代币经济”，它决定了如何处理它的详细信息。为了防止用户（神经元）向 NNS 发送提案，需要支付一定的费用。如果提案被拒绝，则对提交提案的神经元征收费用。
  NNS 通过观察神经元如何发出选票来决定是采用还是拒绝提案。任何人都可以通过锁定 dATT 代币来创建神经元。当用户创建一个神经元时，dATT 的锁定余额只能通过销毁神经元来解锁。我们鼓励用户创建神经元，他们将在对提案进行投票时获得奖励。奖励采用由 NNS 创建的新铸造的 dATT 的形式。分配给神经元的 dATT 奖励数量取决于以下因素：锁定余额的大小、剩余的最小锁定期（“溶解延迟”）、神经元的“年龄”、它参与的可能投票比例、以及所有神经元的投票活动总和。
  在任何时候，每个神经元都有一个当前配置的“溶解延迟”。这决定了将其置于“溶解模式”时溶解所需的时间。一旦一个神经元被置于“溶解模式”，它的溶解延迟会随着时间的推移而下降，就像厨房计时器一样，直到它达到零，然后它的所有者可以执行最后的支付动作来解锁 dATT。解散延迟为神经元所有者提供了一种经济激励，以期在未来某个日期最大化其锁定的 dATT 余额的价值。 dATT 是网络长期价值的代表，它创造了为网络的最佳利益投票的经济激励。神经元拥有者可以自由配置更高的溶解延迟，最长可达八年，但不能减少溶解延迟，除非时间自然流逝。 NNS 支付更高的投票奖励，溶解延迟越高，鼓励用户进入一个根据非常长期的愿景创造经济激励的游戏。
  神经元所有者可能会发现很难直接对提交给 NNS 的每个提案进行手动投票。首先，可能会向 NNS 提交大量提案，神经元的所有者可能没有时间或没有必要的时间来评估每个提案。其次，神经元所有者可能缺乏必要的专业知识来自己评估提案。 NNS 使用一种流动民主的形式来应对这些挑战。对于任何提议主题，一个神经元可以配置为通过跟随一组神经元的投票自动投票，当大多数被跟随者投票通过时投票通过提议，当多数通过变得不可能时投票拒绝。也可以定义一个包罗万象的跟随规则，以自动对具有未定义跟随规则的主题的提案进行神经元投票。假如神经元所有者将根据网络的最佳利益来管理他们的神经元如何跟随其他神经元，这也符合他们自己的经济利益，因为他们锁定了 dATT 余额。
  预计 dATT 整体供应量的很大一部分将被锁定以获得奖励。这使得攻击者获得足够大的股份以获得重大影响既困难又昂贵，从而保护了 ATT 的网络自治。由于神经元所有者可能希望通过对所有提案进行投票来最大化他们的奖励，因此大多数神经元要么被主动管理，要么被配置为跟随其他神经元，以便它们可以自动投票。在实践中，一旦受信任的神经元对提案进行了投票，作为级联跟随关系的结果，大多数其他神经元也将投票。这意味着 NNS 通常可以快速确定所有神经元所代表的总体投票权的大多数是否希望通过或拒绝提案，并据此决定提案。然而，NNS 不能依赖于获得这样的多数，因为原则上，神经元所有者可能不会定义遵循规则，或者他们可能只是选择不投票。`
...
- 格式
  ...
  `每个提交给 NNS 的提案都有以下字段：
  摘要(Summary)：提供提案简短描述的文本，最多使用 256 个字节。
  URL：评估提案所需的附加内容的网址，使用 HTTPS 协议。例如，地址可能描述支持将数据中心 ID 分配给新数据中心的内容。
  提案者(Proposer)：提交提案的 NNS 的 ID。当提案被提交时，如果提案被拒绝，其余额将被收取“费用”。因此，余额需要足够大以支付（所有）拒绝的费用。我们要求神经元有 ≥ 6 个月的溶解延迟才能投票，这也适用于提交提案。
  提案类型(Proposal Type)：提案的类型。这可以推断出它属于哪个主题，如果提案被采纳，将处理该提案的系统函数，以及将传递给该函数的参数的类型和结构。
  参数(Parameters)：将传递给系统函数的参数，如果提议被采纳，系统函数将调用该参数，由其类型决定。当提案被提交时，NNS 检查参数。
  NNS 为其收到的每个提案分配一个唯一标识。
  `
